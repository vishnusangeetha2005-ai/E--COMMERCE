...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const storeUrl = localStorage.getItem('storeUrl') || 'default';
        ...
        <section className="bg-gradient-to-r from-primary-600 to-primary-800 text-white">
          <div className="max-w-7xl mx-auto px-4 py-20 sm:px-6 lg:px-8">
            <div className="max-w-2xl">
              <h1 className="text-4xl md:text-5xl font-bold leading-tight">Discover Amazing Products</h1>
              ...
              <Link href="/products" className="mt-8 inline-flex items-center gap-2 bg-white text-primary-700 px-6 py-3 rounded-lg font-semibold hover:bg-primary-50 transition-colors">
        ...
        {categories.length > 0 && (
          <section className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-wrap gap-3">
              {categories.map(cat => (
                <Link key={cat} href={`/products?category=${cat}`} className="px-5 py-2.5 bg-white border border-gray-200 rounded-full text-sm font-medium text-gray-700 hover:bg-primary-50 hover:border-primary-300 hover:text-primary-700 transition-colors">
                  {cat}
                </Link>
              ))}
            </div>
