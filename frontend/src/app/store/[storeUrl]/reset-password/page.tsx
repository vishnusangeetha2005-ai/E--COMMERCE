...
function StoreResetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.post(`/store/${storeUrl}/reset-password`, { token, password });
      setDone(true);
        ...
        <div className="card">
          {done ? (
            <div className="text-center space-y-4">
              ...
              <Link href={`${base}/login`} className="btn-primary block text-center">Go to Login</Link>
            </div>
          ) : (
            <>
...
export default function StoreResetPasswordPage() {
  return <Suspense><StoreResetPasswordForm /></Suspense>;
}
