      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            {user ? (
              <>
                <Link href="/orders" className="text-gray-600 hover:text-primary-600 transition-colors">Orders</Link>
        ...
        {menuOpen && (
          <div className="md:hidden pb-4 space-y-2">
            <Link href="/products" className="block py-2 text-gray-600" onClick={() => setMenuOpen(false)}>Products</Link>
            {user ? (
              <>
                <Link href="/orders" className="block py-2 text-gray-600" onClick={() => setMenuOpen(false)}>Orders</Link>
                <Link href="/wishlist" className="block py-2 text-gray-600" onClick={() => setMenuOpen(false)}>Wishlist</Link>
                <Link href="/cart" className="block py-2 text-gray-600" onClick={() => setMenuOpen(false)}>Cart</Link>
                <button onClick={() => { logout(); setMenuOpen(false); }} className="block py-2 text-red-500">Logout</button>
              </>
            ) : (
              <>
                <Link href="/login" className="block py-2 text-gray-600" onClick={() => setMenuOpen(false)}>Login</Link>
                <Link href="/register" className="block py-2 text-primary-600 font-medium" onClick={() => setMenuOpen(false)}>Sign Up</Link>
              </>
            )}
          </div>
        )}
